...
public class Dip {
    ...
    static final int MAX_COUNT_STARS = 2;
    static final int MAX_N_STARS = 10;
    static final int MAX_COUNT_NUMBERS = 5;
    static final int MAX_N_NUMBERS = 50;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (MAX_COUNT_NUMBERS == arrayOfNumbers.length && MAX_COUNT_STARS == arrayOfStarts.length) {
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < MAX_COUNT_NUMBERS; ) {
            int candidate = generator.nextInt(MAX_N_NUMBERS-1) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                i++;
            }
        }
        for (int i = 0; i < MAX_COUNT_STARS; ) {
            int candidate = generator.nextInt(MAX_N_STARS-1) + 1;
